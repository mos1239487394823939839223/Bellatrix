...
const TrainingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0E1121] text-white overflow-hidden">
            ...
            <div className="lg:w-1/4">
              <ul className="space-y-4">
                {modules.map((module, index) => (
                  <li key={index}>
                    <button
                      className={`relative w-full text-left p-4 rounded-lg transition-all duration-300 overflow-hidden ${activeModuleIndex === index ? 'bg-[#B38BFF]/20 border-[#B38BFF] text-white transform scale-105 shadow-lg' : 'bg-[#1A1D2E] border-white/10 hover:border-[#B38BFF]/30 hover:scale-102 text-white/80'}`}
                      onClick={() => setActiveModuleIndex(index)}
                    >
                      {activeModuleIndex === index && (
                        <motion.span
                          layoutId="active-tab-indicator"
                          className="absolute inset-0 rounded-lg bg-gradient-to-r from-[#B38BFF] to-[#9B6BFF] opacity-10"
      ...
      <section className="min-h-[50vh] bg-[#0E1121]"></section>
    </div>
  );
};
...
// Placeholder for HeroSection until full file is read
const HeroSection = () => (
